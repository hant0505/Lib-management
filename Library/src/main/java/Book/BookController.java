...
import static Alert.Alert.alertConfirm;
import static Alert.Alert.alertWarning;
...
public class BookController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        button_account.setOnAction(e -> DBUtils.changeScene(e, "/Attendance/test.fxml", "Account"));
        ...
        // change scene
        button_transactions.setOnAction(e -> DBUtils.changeScene(e,"/Transactions/bookTransact.fxml", "Transactions"));
    }
    ...
    private void deleteBook() {
        ...
        if (bookToDelete.isEmpty()) {
            alertWarning("Chưa có sách nào được chọn !");
        } else {
            if (alertConfirm("Bạn có chắc muốn xóa " + bookToDelete.size() + " quyển sách không ?")) {
    ...
    private void updateBook() {
        ...
        if (selectedBooks.size() != 1) {
            alertWarning("Vui lòng chọn một sách duy nhất để cập nhật !");
            return;
        }
        ...
        try {
            ...
            if (selected == null) {
                alertWarning("Hãy chọn sách cần cập nhật !");
            }
